    ...
    <span className="text-green-700 font-bold text-lg">
      ৳ {item.price}
        ...
        {!cartItem ? (
          ...
          <motion.div
  className="mt-4 flex items-center justify-center bg-green-50 border border-green-200 rounded-full py-2 px-4 gap-4 shadow-sm"
  initial={{
    opacity: 0,
    y: 10,
  }}
...
>
  <button
    className="w-7 h-7 flex items-center justify-center rounded-full bg-green-100 hover:bg-green-200 transition-all cursor-pointer active:scale-95"
    onClick={() => dispatch(decreaseQuantity(item._id))}
  >
    <Minus size={16} className="text-green-700" />
  </button>

  <span className="text-sm font-semibold text-green-800 min-w-[20px] text-center">
    {cartItem.quantity}
  </span>
